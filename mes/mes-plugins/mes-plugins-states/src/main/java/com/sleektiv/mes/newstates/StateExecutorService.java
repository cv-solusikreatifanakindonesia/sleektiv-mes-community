...
import com.sleektiv.view.api.ComponentMessagesHolder;
...
public class StateExecutorService {
    ...
    public <M extends StateService> void changeState(final Class<M> serviceMarker, final ViewDefinitionState view,
        ...
        Long userId = securityService.getCurrentUserOrSleektivBotId();
        String userName = securityService.getCurrentUserOrSleektivBotName();

        Optional<GridComponent> maybeGridComponent = view.tryFindComponentByReference(SleektivViewConstants.L_GRID);
    ...
    public Entity buildStateChangeEntity(final StateChangeEntityDescriber describer, final Entity owner, String userName,
        ...
        if (StringUtils.isEmpty(userName)) {
            userName = securityService.getCurrentUserOrSleektivBotName();
        }
