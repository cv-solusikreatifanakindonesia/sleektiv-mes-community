...
import com.sleektiv.mes.masterOrders.constants.SalesVolumeMultiFields;
...
public class SalesVolumeAddMultiListeners {
    ...
    public final void createSalesVolumes(final ViewDefinitionState view, final ComponentState state, final String[] args) {
        FormComponent salesVolumeMultiForm = (FormComponent) view.getComponentByReference(SleektivViewConstants.L_FORM);
    ...
    private BigDecimal getBigDecimal(final FieldComponent quantityField, boolean isRequired) {
        ...
        if (eitherQuantity.isRight()) {
            if (eitherQuantity.getRight().isPresent()) {
                ...
                if (valueScale > scale) {
                    quantityField.addMessage("sleektivView.validate.field.error.invalidScale.max", ComponentState.MessageType.FAILURE, String.valueOf(scale));

                    return null;
                }

                if (BigDecimal.ZERO.compareTo(quantity) >= 0) {
                    quantityField.addMessage("sleektivView.validate.field.error.outOfRange.toSmall", ComponentState.MessageType.FAILURE);

                    return null;
                }
            } else {
                if (isRequired) {
                    quantityField.addMessage("sleektivView.validate.field.error.missing", ComponentState.MessageType.FAILURE);
                }

                return null;
            }
        } else {
            quantityField.addMessage("sleektivView.validate.field.error.invalidNumericFormat", ComponentState.MessageType.FAILURE);
