...
import com.sleektiv.mes.masterOrders.constants.ProductsByAttributeEntryHelperFields;
import com.sleektiv.mes.masterOrders.constants.ProductsByAttributeHelperFields;
import com.sleektiv.mes.technologies.tree.ProductStructureTreeService;
...
public class ProductsByAttributeListeners {
    ...
    private void clearProductsList(final ViewDefinitionState view) {
        FormComponent productsByAttributeHelperForm = (FormComponent) view.getComponentByReference(SleektivViewConstants.L_FORM);
    ...
    public void addPositionsToOrder(final ViewDefinitionState view, final ComponentState state, final String[] args) {
        ...
        if (Objects.isNull(productLookup.getEntity())) {
            productLookup.addMessage("sleektivView.validate.field.error.missing", ComponentState.MessageType.FAILURE);
    ...
    private BigDecimal getBigDecimal(final FieldComponent orderedQuantityField) {
        ...
        if (eitherOrderedQuantity.isRight()) {
            if (eitherOrderedQuantity.getRight().isPresent()) {
                ...
                if (valueScale > scale) {
                    orderedQuantityField.addMessage("sleektivView.validate.field.error.invalidScale.max", ComponentState.MessageType.FAILURE, String.valueOf(scale));

                    return null;
                }

                if (BigDecimal.ZERO.compareTo(orderedQuantity) > 0) {
                    orderedQuantityField.addMessage("sleektivView.validate.field.error.outOfRange.toSmall", ComponentState.MessageType.FAILURE);

                    return null;
                }
            } else {
                orderedQuantityField.addMessage("sleektivView.validate.field.error.missing", ComponentState.MessageType.FAILURE);

                return null;
            }
        } else {
            orderedQuantityField.addMessage("sleektivView.validate.field.error.invalidNumericFormat", ComponentState.MessageType.FAILURE);
