...
import com.sleektiv.mes.basic.ShiftsServiceImpl;
...
public class OrderTimePredictionListeners {
    ...
    public void changeRealizationTime(final ViewDefinitionState view, final ComponentState state, final String[] args) {
        ...
        if (value instanceof BigDecimal) {
            ...
            plannedQuantityField.addMessage("sleektivView.validate.field.error.invalidNumericFormat", MessageType.FAILURE);
            return;
        } else {
            try {
        ...
        if (scale > MAX) {
            plannedQuantityField.addMessage("sleektivView.validate.field.error.invalidScale.max", MessageType.FAILURE,
                    MAX.toString());
            return;
        }
        ...
        if (presicion > MAX) {
            plannedQuantityField.addMessage("sleektivView.validate.field.error.invalidPrecision.max", MessageType.FAILURE,
                    MAX.toString());
            return;
        }

        if (BigDecimal.ZERO.compareTo(quantity) >= 0) {
            plannedQuantityField.addMessage("sleektivView.validate.field.error.outOfRange.toSmall", MessageType.FAILURE);
            return;
        }
