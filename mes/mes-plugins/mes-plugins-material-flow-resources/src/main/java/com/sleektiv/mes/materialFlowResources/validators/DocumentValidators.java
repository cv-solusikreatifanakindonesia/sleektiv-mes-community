...
import com.sleektiv.mes.materialFlowResources.service.DocumentService;
import com.sleektiv.mes.materialFlowResources.service.ResourceManagementService;
    ...
    public void validatePositionsAndCreateResources(final ComponentState documentForm, final Entity document) {
        if (!document.getHasManyField(DocumentFields.POSITIONS).isEmpty()) {
            if (validatePositions(document)) {
                ...
                documentForm.addMessage("sleektivView.validate.global.error.custom", ComponentState.MessageType.FAILURE);
            }
        } else {
            document.setNotValid();
