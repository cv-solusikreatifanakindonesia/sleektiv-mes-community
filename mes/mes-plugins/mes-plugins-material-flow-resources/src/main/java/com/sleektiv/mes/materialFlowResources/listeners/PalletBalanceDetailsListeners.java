...
import com.sleektiv.mes.materialFlowResources.palletBalance.PalletBalanceReportHelper;
import com.sleektiv.mes.materialFlowResources.palletBalance.PalletBalanceXlsService;
...
public class PalletBalanceDetailsListeners {
    ...
    public void generatePalletBalance(final ViewDefinitionState viewDefinitionState, final ComponentState state,
        ...
        if (!state.isHasError()) {
            ...
            if (report == null) {
                state.addMessage("sleektivView.message.entityNotFound", ComponentState.MessageType.FAILURE);
                return;
