...
import com.sleektiv.mes.basic.constants.ExpiryDateValidityUnit;
...
import com.sleektiv.mes.orders.states.constants.OrderStateChangeDescriber;
...
import com.sleektiv.view.api.utils.TimeConverterService;
        ...
        if (datesChanged && !orderState.equals(OrderState.PENDING)) {
            ...
            if (StringUtils.isEmpty(workerToChange)) {
                orderStateChange.setField(OrderStateChangeFields.WORKER,
                        securityService.getCurrentUserOrSleektivBotName());
            } else {
        ...
        if (plannedQuantityFromDB.compareTo(plannedQuantity) != 0) {
            ...
            if (Objects.nonNull(commissionedCorrectedQuantity)) {
                if (commissionedCorrectedQuantity.compareTo(BigDecimal.ZERO) == 0) {
                    order.addError(orderDD.getField(OrderFields.COMMISSIONED_CORRECTED_QUANTITY), "sleektivView.validate.field.error.outOfRange.toSmall");
                }
                ...
                if (commissionedPlannedQuantity.compareTo(BigDecimal.ZERO) == 0) {
                    order.addError(orderDD.getField(OrderFields.COMMISSIONED_PLANNED_QUANTITY), "sleektivView.validate.field.error.outOfRange.toSmall");
                }
                order.setField(OrderFields.PLANNED_QUANTITY,
