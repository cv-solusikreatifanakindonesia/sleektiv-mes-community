...
import com.sleektiv.mes.orders.constants.OrderTechnologicalProcessPartFields;
        ...
        if (orderTechnologicalProcessPartForms.size() < 2) {
            ...
            for (FormComponent orderTechnologicalProcessPartForm : orderTechnologicalProcessPartForms) {
                ...
                if (eitherQuantity.isLeft()) {
                    quantityField.addMessage("sleektivView.validate.field.error.custom", ComponentState.MessageType.FAILURE);
                    ...
                    if (BigDecimal.ZERO.compareTo(quantityPart) >= 0) {
                        quantityField.addMessage("sleektivView.validate.field.error.outOfRange.toSmall",
                                ComponentState.MessageType.FAILURE);
                        ...
                        if (scale > max) {
                            quantityField.addMessage("sleektivView.validate.field.error.invalidScale.max",
