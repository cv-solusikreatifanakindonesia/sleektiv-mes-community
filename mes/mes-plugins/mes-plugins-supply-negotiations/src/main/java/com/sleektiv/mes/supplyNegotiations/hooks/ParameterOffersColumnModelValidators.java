...
@Service
public class ParameterOffersColumnModelValidators {

    @Autowired
    private DataDefinitionService dataDefinitionService;

    public final boolean checkIfColumnForOffersIsNotAlreadyUsed(final DataDefinition ordersColumnDD, final Entity ordersColumn) {
        return checkIfColumnIsNotUsed(ordersColumnDD, ordersColumn, "parameter", "columnForOffers", "parameterColumnForOffers");
                ...
                if (model == null) {
                    ...
                    for (Entity columnUsed : dataDefinitionService
                            .get(SupplyNegotiationsConstants.PLUGIN_IDENTIFIER,
                                    SupplyNegotiationsConstants.MODEL_PARAMETER_COLUMN_FOR_OFFERS).find().list().getEntities()) {

                        if (columnUsed.getBelongsToField("columnForOffers").getId().equals(column.getId())) {
