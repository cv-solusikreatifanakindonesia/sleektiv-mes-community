...
<xsl:stylesheet version="1.0"
	...
	xmlns:qcd="http://schema.sleektiv.org/model"
	...
	<xsl:template name="entityName">
		...
		<xsl:attribute name="class">
			<xsl:value-of
			select="concat('com.sleektiv.model.beans.', $pluginName, '.', translate(substring($pluginName, 1, 1),  $smallcase, $uppercase), substring($pluginName, 2), translate(substring($modelName, 1, 1),  $smallcase, $uppercase), substring($modelName, 2))" />
		</xsl:attribute>
	</xsl:template>
		...
		<hibernate-mapping>
			<class>
				...
				<xsl:attribute name="name">
					<xsl:value-of
					select="concat('com.sleektiv.model.beans.', @plugin, '.', translate(substring(@plugin, 1, 1),  $smallcase, $uppercase), substring(@plugin, 2), translate(substring(@name, 1, 1),  $smallcase, $uppercase), substring(@name, 2))" />
				</xsl:attribute>
