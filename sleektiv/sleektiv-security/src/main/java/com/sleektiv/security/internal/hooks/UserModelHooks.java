 ...
 * ***************************************************************************
 */
package com.sleektiv.security.internal.hooks;

import com.sleektiv.localization.api.TranslationService;
import com.sleektiv.mail.api.MailService;
...
public class UserModelHooks {
    ...
    public boolean validateGroupChangeDate(final DataDefinition userDD, final Entity user) {
        ...
        if (Objects.isNull(userId)) {
            ...
            if (!securityService.hasCurrentUserRole(SleektivSecurityConstants.ROLE_SUPERADMIN) && user.getDateField(UserFields.GROUP_CHANGE_DATE).toInstant().atZone(ZoneId.systemDefault()).toLocalDate().isAfter(LocalDate.now().minusDays(7))) {
                user.addError(userDD.getField(UserFields.GROUP), "sleektivUsers.validate.global.error.forbiddenGroupChange");
