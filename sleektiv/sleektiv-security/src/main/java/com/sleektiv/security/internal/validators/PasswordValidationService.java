    ...
    public boolean checkPassword(final DataDefinition userDD, final Entity user) {
        ...
        if ("profileChangePassword".equals(viewIdentifier)) {
            if (Objects.isNull(oldPassword)) {
                user.addError(userDD.getField(UserFields.OLD_PASSWORD), "sleektivUsers.validate.global.error.noOldPassword");
            ...
            if (!passwordEncoder.matches(oldPassword, currentPassword.toString())) {
                user.addError(userDD.getField(UserFields.OLD_PASSWORD), "sleektivUsers.validate.global.error.wrongOldPassword");
        ...
        if (Objects.isNull(password)) {
            user.addError(userDD.getField(UserFields.PASSWORD), "sleektivUsers.validate.global.error.noPassword");
        ...
        if (Objects.isNull(passwordConfirmation)) {
            user.addError(userDD.getField(UserFields.PASSWORD_CONFIRMATION),
                    "sleektivUsers.validate.global.error.noPasswordConfirmation");

            return false;
        }

        if (!passwordEncoder.matches(passwordConfirmation, password)) {
            user.addError(userDD.getField(UserFields.PASSWORD), "sleektivUsers.validate.global.error.notMatch");
            user.addError(userDD.getField(UserFields.PASSWORD_CONFIRMATION), "sleektivUsers.validate.global.error.notMatch");
