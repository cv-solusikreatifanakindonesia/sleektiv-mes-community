...
import com.sleektiv.view.internal.FilterValueHolderImpl;
        ...
        private void reload() {
            if (Objects.isNull(belongsToFieldDefinition) || Objects.nonNull(belongsToEntityId)) {
                ...
                try {
                    ...
                    addMessage("sleektivView.grid.filter.incorrectValue", MessageType.FAILURE, gcfe.getFilterValue());
            ...
            if (column.getFields().get(0).getType() instanceof EnumType) {
                ...
                if (Objects.nonNull(fieldValue)) {
                    if (fieldValue.equals("1")) {
                        values.put(column.getName(), getTranslationService().translate("sleektivView.true", getLocale()));
                    } else {
                        values.put(column.getName(), getTranslationService().translate("sleektivView.false", getLocale()));
                    }
                } else {
                    values.put(column.getName(), "");
                }
