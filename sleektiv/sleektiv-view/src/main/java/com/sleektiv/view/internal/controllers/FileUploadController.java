...
public class FileUploadController {
    ...
    public ModelAndView upload(final Locale locale) {
        ModelAndView mav = getCrudPopupView(SleektivViewConstants.VIEW_FILE_UPLOAD, locale);

        viewParametersAppender.appendCommonViewObjects(mav);

        mav.addObject("headerLabel", translationService.translate("sleektivView.fileUpload.header", locale));
        mav.addObject("buttonLabel", translationService.translate("sleektivView.fileUpload.button", locale));
        mav.addObject("chooseFileLabel", translationService.translate("sleektivView.fileUpload.chooseFileLabel", locale));
        mav.addObject("maxUploadSizeExceeded",
                translationService.translate("sleektivView.errorPage.error.uploadException.maxSizeExceeded.explanation", locale, "" + maxUploadSize / 1000000));
    ...
    private ModelAndView getCrudPopupView(final String viewName, final Locale locale) {
        ...
        return crudController.prepareView(SleektivViewConstants.PLUGIN_IDENTIFIER, viewName, crudArgs, locale);
    }
